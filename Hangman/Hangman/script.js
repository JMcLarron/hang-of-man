...
$(document).ready(function () {
    ...
    switch (true) {
        ...
        case (randArray === chosenArray[1]):
            $("#category").html(" Mythical Creatures")
            break;
        case (randArray === chosenArray[2]):
            $("#category").html(" Organs")
        default:
    ...
    $(document).keyup(function (letter) {
        ...
        //add letter chosen to letters guessed array
      console.log()
      let selected = false
        if (lettGuessed.indexOf(letter.key) === -1) {
            lettGuessed.push(letter.key)
            selected = true
        }

        if (selected === false) {
            alert ("Select another letter Geez Louis!")
        }
        ...
        for (let x = 0; x < splitWord.length; x++) {
            if (splitWord[x] === letter.key && selected === true) {
                correct = true
                corr++
                console.log(corr)
        ...
        if (correct === false && selected === true) {
            bodyParts--;
            $("#body_parts").html(bodyParts)
